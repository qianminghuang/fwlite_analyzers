...
#        continue
    ...
    for i in range(0,len(lheinfo.product().hepeup().IDUP)):
        ...
        if lheinfo.product().hepeup().IDUP.at(i) == 22:
            if abs(lheinfo.product().hepeup().IDUP.at(lheinfo.product().hepeup().MOTHUP.at(i)[0]-1)) == 24:
                fsr = True
...
#    if v_neutrino.Pt() > 60 and deltaR(v_photon.Eta(),v_photon.Phi(),v_lepton.Eta(),v_lepton.Phi()) > 0.5 and fsr:        
#    if v_neutrino.Pt() > 60 and fsr:
#    if v_neutrino.Pt() > 50 and not fsr:
#        h.Fill(abs(deltaPhi(v_lepton.Phi(),v_photon.Phi())))

#        print str(abs(deltaPhi(v_lepton.Phi(),v_photon.Phi()))) + " " + str(lheinfo.product().hepeup().PUP.at(2)[0])
        
    if v_photon.M() == 0 or v_lepton.M() == 0 or v_neutrino.M() == 0:
        continue

    if not fsr:         
        h.Fill(v_neutrino.Pt())
